...
![Autonomous DFIR Agent architecture](architecture-diagram.png)

User → local Web UI → the **DFIR agent / orchestrator** (LLM-guided reasoning +
**deterministic** workflow nodes) → the **read-only MCP safety boundary** (typed tools, *no
shell*, *no write to evidence*, path-validated, provenance-logged) → court-vetted **SIFT
tools** → **read-only evidence**. The LLM explains the case but **never accesses raw evidence
directly**; every output is a **structured, cited finding** backed by the provenance ledger.
